...
#include "MarlinCore.h" //导入Marlin核心固件的声明

#include "HAL/shared/Delay.h"                        //导入延时函数
#include "HAL/shared/esp_wifi.h"                     //导入WiFi驱动
#include "HAL/shared/cpu_exception/exception_hook.h" //导入异常钩子

#ifdef ARDUINO            // 如果芯片是Arduino
#include <pins_arduino.h> //导入Arduino引脚定义
#endif

#include <math.h> //C语言库函数 数学函数

// 导入模块固件（外设）
#include "module/endstops.h"     //终点挡板
#include "module/motion.h"       //运动/移动 估计是步进电机的驱动
#include "module/planner.h"      //计划 具体未知
#include "module/printcounter.h" // PrintCounter or Stopwatch
#include "module/settings.h"     //设置或者设置界面
#include "module/stepper.h"      //步进电机
#include "module/temperature.h"  //温度

// 切片文件
#include "gcode/gcode.h"  //切片文件（具体未知）
#include "gcode/parser.h" //分析程序
#include "gcode/queue.h"  //队列

#include "feature/pause.h" //特征--暂停
#include "sd/cardreader.h" //SD卡--读卡器
#include "lcd/marlinui.h"  //LCD屏--马林UI

#if HAS_TOUCH_BUTTONS                // 如果定义了 HAS 触摸按键
#include "lcd/touch/touch_buttons.h" //导入lcd触摸驱动
#endif

#if HAS_TFT_LVGL_UI                                  // TFT屏（具体未知）退出UI或者外置UI文件夹下的mkd_ui
#include "lcd/extui/mks_ui/tft_lvgl_configuration.h" //屏幕lvgl配置
#include "lcd/extui/mks_ui/draw_ui.h"                //屏幕绘画
#include "lcd/extui/mks_ui/mks_hardware.h"           //硬件设备
#include <lvgl.h>                                    //未知
#endif

#if HAS_DWIN_E3V2 // 未知

#include "lcd/e3v2/common/encoder.h" //共享--编码器

#if ENABLED(DWIN_CREALITY_LCD)           // 使能双色液晶
#include "lcd/e3v2/creality/dwin.h"      //未知
#elif ENABLED(DWIN_LCD_PROUI)            // 未知
#include "lcd/e3v2/proui/dwin.h"         //未知
#elif ENABLED(DWIN_CREALITY_LCD_JYERSUI) // 未知
#include "lcd/e3v2/jyersui/dwin.h"       //未知
#endif
#endif
...
#if ENABLED(IIC_BL24CXX_EEPROM) // 使能IIC EEPROM驱动函数
#include "libs/BL24CXX.h"       //导入EEPROM驱动函数
#endif

#if ENABLED(DIRECT_STEPPING) // 直接步近
#include "feature/direct_stepping.h"
#endif

#if ENABLED(HOST_ACTION_COMMANDS) // 主机动作命令
#include "feature/host_actions.h" //特征--主机动作
#endif

#if HAS_BEEPER
#include "libs/buzzer.h" //导入蜂鸣器
#endif

#if ENABLED(EXTERNAL_CLOSED_LOOP_CONTROLLER) // 使能（关闭外置循环控制器）
#include "feature/closedloop.h"              //关闭循环
#endif

#if HAS_MOTOR_CURRENT_I2C            // 电机电流IIC
#include "feature/digipot/digipot.h" //导入数字电位器库文件
#endif

#if ENABLED(MIXING_EXTRUDER) // 使能（混合喷头）
#include "feature/mixing.h"  //混合
#endif

#if ENABLED(MAX7219_DEBUG)   // 某东西的调试
#include "feature/max7219.h" //多位LED 显示驱动器
#endif

#if HAS_COLOR_LEDS             // LED颜色控制
#include "feature/leds/leds.h" //导入LED库文件
#endif

#if ENABLED(BLTOUCH) // 未知（猜测是蓝牙，单词拼写错误）
#include "feature/bltouch.h"
#endif

#if ENABLED(BD_SENSOR)                // 热床传感器
#include "feature/bedlevel/bdl/bdl.h" //热床高度传感器
#endif

#if ENABLED(POLL_JOG)         // 未知
#include "feature/joystick.h" //控制手柄
#endif

#if HAS_SERVOS            // 伺服或者舵机
#include "module/servo.h" //导入伺服驱动文件
#endif

#if HAS_MOTOR_CURRENT_DAC            // 电流DAC
#include "feature/dac/stepper_dac.h" //步进电机DAC
#endif

#if ENABLED(EXPERIMENTAL_I2CBUS) // 实验性IIC总线
#include "feature/twibus.h"
#endif

#if ENABLED(I2C_POSITION_ENCODERS) // IIC位置编码器
#include "feature/encoder_i2c.h"   //IIC编码器
#endif

#if (HAS_TRINAMIC_CONFIG || HAS_TMC_SPI) && DISABLED(PSU_DEFAULT_OFF) // 关闭默认供电
#include "feature/tmc_util.h"
#endif

#if HAS_CUTTER                     // 切割刀
#include "feature/spindle_laser.h" //导入轴激光库文件
#endif

#if ENABLED(SDSUPPORT) // 支持32G
CardReader card;       // 读卡器
#endif

#if ENABLED(G38_PROBE_TARGET) // G38探头目标
uint8_t G38_move;             // = 0
bool G38_did_trigger;         // = false
#endif

#if ENABLED(DELTA) // 德尔塔（作用未知）
...
#if ENABLED(GCODE_REPEAT_MARKERS) // G命令重复标记
#include "feature/repeat.h"
#endif

#if ENABLED(POWER_LOSS_RECOVERY) // 电源丢失移除
#include "feature/powerloss.h"   //电力损失
#endif

#if ENABLED(CANCEL_OBJECTS) // 取消对象
#include "feature/cancel_object.h"
#endif

#if HAS_FILAMENT_SENSOR     // 丝传感器
#include "feature/runout.h" //跳动度
#endif

#if EITHER(PROBE_TARE, HAS_Z_SERVO_PROBE) // Z轴伺服传感器
#include "module/probe.h"                 //探针
#endif

#if ENABLED(HOTEND_IDLE_TIMEOUT) // 加热空闲超时
#include "feature/hotend_idle.h"
#endif

#if ENABLED(TEMP_STAT_LEDS)        // 温控器指示灯
#include "feature/leds/tempstat.h" //温控器库文件
#endif

#if ENABLED(CASE_LIGHT_ENABLE) // 机箱灯使能
...
#include "module/tool_change.h" //工具变化

#if HAS_FANCHECK
#include "feature/fancheck.h"
#endif

#if ENABLED(USE_CONTROLLER_FAN) // （使用/用户）风扇控制器
#include "feature/controllerfan.h"
#endif

#if HAS_PRUSA_MMU1           // 存储器管理单元1
#include "feature/mmu/mmu.h" //存储器管理单元1
#endif

#if HAS_PRUSA_MMU2 // 存储器管理单元2
#include "feature/mmu/mmu2.h"
#endif

#if ENABLED(PASSWORD_FEATURE) // 密码特点
...
#if HAS_DRIVER_SAFE_POWER_PROTECT          // 驱动器安全电源保护
#include "feature/stepper_driver_safety.h" //导入步进驱动器保护
#endif

#if ENABLED(PSU_CONTROL) // 电源控制
#include "feature/power.h"
#endif

#if ENABLED(EASYTHREED_UI) // 简易的三维UI
#include "feature/easythreed_ui.h"
#endif

#if ENABLED(MARLIN_TEST_BUILD) // 马林测试构建
#include "tests/marlin_tests.h"
#endif

PGMSTR(M112_KILL_STR, "M112 Shutdown"); // 某个关闭

MarlinState marlin_state = MF_INITIALIZING; // 初始化中

// For M109 and M190, this flag may be cleared (by M108) to exit the wait loop
// 对于M109和M190，这个标志可以被M108清除以退出等待循环
bool wait_for_heatup = true;

// For M0/M1, this flag may be cleared (by M108) to exit the wait-for-user loop
// 对于M0/M1，该标志可以被M108清除以退出wait-for-user循环
...
/**
 * @description: 失能步进电机驱动
 * @param {*}
 * @return {*}
 */
void disableStepperDrivers()
{
    OUT_WRITE(STEPPER_RESET_PIN, LOW); // 电机复位引脚拉低
}
// Drive down to keep motor driver chips in reset
// 拉低引脚以保持电机驱动器芯片复位

/**
 * @description: 使能步进电机驱动
 * @param {*}
 * @return {*}
 */
void enableStepperDrivers()
{
    SET_INPUT(STEPPER_RESET_PIN); // 设置为输入，允许将引脚拉高
} // Set to input, allowing pullups to pull the pin high
 ...
 *  -检查自定义用户按钮按下
 *  -检查冷却风扇是否需要打开
 *  -检查空闲但热挤出机是否需要挤出丝(EXTRUDER_RUNOUT_PREVENT)
 *  -脉冲fet_safey_pin如果它存在
 **/
...
#define CHECK_BETTER_USER_BUTTON(N) _CHECK_CUSTOM_USER_BUTTON(N, if (strlen(BUTTON##N##_DESC)) LCD_MESSAGE_F(BUTTON##N##_DESC))
    ...
    if (thermalManager.degHotend(active_extruder) > (EXTRUDER_RUNOUT_MINTEMP) && ELAPSED(ms, gcode.previous_move_ms + SEC_TO_MS(EXTRUDER_RUNOUT_SECONDS)) && !planner.has_blocks_queued())
    {
        ...
        {
        default:
            oldstatus = stepper.AXIS_IS_ENABLED(E_AXIS, 0);
            stepper.ENABLE_EXTRUDER(0);
            break;
#if E_STEPPERS > 1
        case 2:
        case 3:
            oldstatus = stepper.AXIS_IS_ENABLED(E_AXIS, 1);
            stepper.ENABLE_EXTRUDER(1);
            break;
#if E_STEPPERS > 2
        case 4:
        case 5:
            oldstatus = stepper.AXIS_IS_ENABLED(E_AXIS, 2);
            stepper.ENABLE_EXTRUDER(2);
            break;
#if E_STEPPERS > 3
        case 6:
        case 7:
            oldstatus = stepper.AXIS_IS_ENABLED(E_AXIS, 3);
            stepper.ENABLE_EXTRUDER(3);
            break;
#endif // E_STEPPERS > 3
#endif // E_STEPPERS > 2
#endif // E_STEPPERS > 1
...
#define _CASE_EN(N)                                     \
    case N:                                             \
        oldstatus = stepper.AXIS_IS_ENABLED(E_AXIS, N); \
...
void idle(bool no_stepper_sleep /*=false*/)
{
    ...
    if (++idle_depth > 5)
        SERIAL_ECHOLNPGM("idle() call depth: ", idle_depth);
#endif
...
// Run StallGuard endstop checks
    ...
    if (endstops.tmc_spi_homing_check())
        break; // Read SGT 4 times per idle loop
#endif
...
void minkill(const bool steppers_off /*=false*/)
{
    ...
    for (;;)
        hal.watchdog_refresh(); // Wait for RESET button or power-cycle
...
void setup()
{
    ...
    // 低功耗待机引脚必须提前设置，否则不可用（具体意义不明）

    // Check startup - does nothing if bootloader sets MCUSR to 0
    // 检查启动-如果引导加载程序将MCUSR设置为0，则不执行任何操作

    const byte mcu = hal.get_reset_source(); // 获取重置源
    hal.clear_reset_source();                // 清除复位源
...
#define SETUP_RUN(C)             \
    ...
    while (!MYSERIAL1.connected() && PENDING(millis(), serial_connect_timeout))
    { /*nada*/
    }
...
#define BAUDRATE_2 BAUDRATE
    ...
    while (!MYSERIAL2.connected() && PENDING(millis(), serial_connect_timeout))
    { /*nada*/
    }
#ifdef SERIAL_PORT_3
#ifndef BAUDRATE_3
...
#if ENABLED(CUSTOM_USER_BUTTONS)
#define INIT_CUSTOM_USER_BUTTON_PIN(N)                  \
    do                                                  \
    {                                                   \
        SET_INPUT(BUTTON##N##_PIN);                     \
        WRITE(BUTTON##N##_PIN, !BUTTON##N##_HIT_STATE); \
    } while (0)
 ...
 *
 * 调用idle()来处理G-code命令之间的所有任务
 * 注意，在idle()期间，队列中的g码不能被执行。
 * 但许多g代码可以像宏一样在任何时候直接调用。
 *
...
/**
 * @description: 马林主程序循环
 * marlin注释：调用idle()来处理G-code命令之间的所有任务
 * idle执行期间无法执行G-codes队列，但许多g代码可以像宏一样在任何时候直接调用。
 * 由此可知：G-code是一堆指令，存在一个队列存放这些指令且按顺序执行。指令执行间隔会调用idle()这个函数。
 *
 * Core Marlin activities（填充队列）、管理加热器、看门狗、内存卡拔插处理...    （先搁置）
 * @param {*}
 * @return {*}
 */
void loop()
...
// sd卡用于更新固件、存放打印模型（G-code）文件。
        ...
        endstops.event_handler(); // 终点.事件处理
        // endstops是管理 endstops 的单例对象。
        // 猜测这个是打印结束的回调函数，或者是G-codes队列每一步执行完毕的回调。（搁置）

        TERN_(HAS_TFT_LVGL_UI, printer_state_polling()); // 判断TFT屏幕是否打印轮询打印状态

        TERN_(MARLIN_TEST_BUILD, runPeriodicTests()); // 判断马林固件测试编译，运行周期性测试
