...
#if ENABLED(SWITCHING_EXTRUDER)       // 定义挤出机开关使能
#define SWITCHING_EXTRUDER_SERVO_NR 0 // 定义 挤出机开关私服系统函数 值为 0 低电平 详见此函数库文件
...
// #define SWITCHING_NOZZLE //定义启动喷嘴开关即可启用，不常见
#if ENABLED(SWITCHING_NOZZLE) // 定义启动喷嘴开关即可启用，不常见
#define SWITCHING_NOZZLE_SERVO_NR 0
// #define SWITCHING_NOZZLE_E1_SERVO_NR 1 //  If two servos are used, the index of the second // 如果使用两个伺服，则第二个伺服的索引
#define SWITCHING_NOZZLE_SERVO_ANGLES \
    {                                 \
        0, 90                         \
    }
// Angles for E0, E1 (single servo) or lowered/raised (dual servo)
// E0、E1角度(单伺服)或升降角度(双伺服)
 ...
 */
// #define PARKING_EXTRUDER //定义 挤出机停车
 ...
 */
// #define MAGNETIC_PARKING_EXTRUDER //定义 磁力停车挤出机 注释掉即可使用

#if EITHER(PARKING_EXTRUDER, MAGNETIC_PARKING_EXTRUDER) // 停车挤压机、磁性停车挤压机 二选一
...
// (mm) Distance to move beyond the parking point to grab the extruder
// 定义 停车挤出机 抓取距离 为 1 (毫米)移动到停车点以外以抓取挤出机的距离

#if ENABLED(PARKING_EXTRUDER) // 定义 停车挤出机使能开关
...
// 定义 停车挤出机螺线管管脚有效 LOW 低电平

#define PARKING_EXTRUDER_SOLENOIDS_DELAY 250
// (ms) Delay for magnetic field. No delay if 0 or not defined.
...
// 定义磁力挤出机最快速度 9000  (毫米/分钟)
// 最后一个距离点之前的行驶速度

#define MPE_SLOW_SPEED 4500
// (mm/min) Speed for last distance travel to park and couple
// 定义磁力挤出机最慢速度      4500 (毫米/分钟)
// 最后一次停车和行程距离的速度

#define MPE_TRAVEL_DISTANCE 10
// (mm) Last distance point
// 定义 磁力挤出机行程距离  最后距离点收工远离点
// 因为挤出头有温度，停留过近会造成模型融化

#define MPE_COMPENSATION 0
// Offset Compensation -1 , 0 , 1 (multiplier) only for coupling
// 定义 磁力挤出机偏移补偿 低电平 0 启动配置 偏移补偿-1，0，1(乘法器)，仅用于耦合
 ...
 * 支持可切换和停靠的工具头，如E3D工具更换器。工具头用伺服锁定。
 * 
 */
